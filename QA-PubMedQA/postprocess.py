# Copyright (c) Microsoft Corporation.
# Licensed under the MIT License.

import sys
import re


out_file = sys.argv[1]

prefix = [
    '(learned[0-9]+ )+',
    'we can conclude that',
    'we have that',
    'in conclusion,',
    
    ]


def strip_prefix(line):
    for p in prefix:
        res = re.search(p, line)
        if res is not None:
            line = re.split(p, line)[-1].strip()
            break
    return line
# def strip_prefix(line):
#     for p in prefix:
#         res = re.search(p, line)
#         if res is not None:
#             line = re.split(p, line)[-2].strip().split('.')[0]
#             break
#     return line

# def convert_relis_sentence(sentence):
#     ans = None
#     sent = sentence.split('.')[-2].strip()
#     return sent

def convert_relis_sentence(sentence):
    ans = None
    #segs = re.search(r"From the context, the question can be answered by(.*)", sentence)
    #segs = re.search(r"From the answer, the answer to the question is(.*)", sentence)

    #segs = re.search(r"From the answer the question can be replied by(.*)", sentence)
    
    #Ori prompt in BioGPT
    segs = re.search(r"the answer to the question given the context is(.*)", sentence)
    if segs is not None:
        #print(segs)
        segs = segs.groups()
        ans = segs[0].strip()
        if ans.endswith('.'):
            ans = ans[:-1]
        #print(ans)
    return ans


all_lines = []
with open(out_file, "r", encoding="utf8") as fr:
    for line in fr:
        e = line.strip()
        if len(e) > 0 and e[-1] == ".":
            all_lines.append(e[:-1])
        else:
            all_lines.append(e)


hypothesis = []
cnt = 0
fail_cnt = 0


for i, line in enumerate(all_lines):
    cnt += 1
    strip_line = strip_prefix(line)
    ans = convert_relis_sentence(strip_line)
    if ans is not None:
        hypothesis.append(ans)
    else:
        hypothesis.append("failed")
        fail_cnt += 1
        print("Failed:id:{}, line:{}".format(i+1, line))


with open(f"{out_file}.extracted.txt", "w", encoding="utf8") as fw:
    for eg in hypothesis:
        print(eg, file=fw)


print(f"failed = {fail_cnt}, total = {cnt}")
